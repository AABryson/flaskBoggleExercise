...
# create instance object near top of app.py; boggle_game = Boggle(); substitute boggle_game for 'self' below
class Boggle():
#initializer
    def __init__(self):
#--call read_dict method and assign to variable words
#--calls method during initialization
        self.words = self.read_dict("words.txt")

    def read_dict(self, dict_path):
        """Read and return all words in dictionary."""
#--pass in a dict_path; a.b. like opening python file from terminal?
        dict_file = open(dict_path)
#--iterate over each line in file; remove leading and trailing white spaces; review
#--store cleaned lines in words;
...
#--will loop over 0-4
        for y in range(5):
#--list comprehension; create new list called 'row'; 5 randomly chosen upper case letters; review
#--string.acsii_uppercase is a string containing all uppercase letters from alphabet; for each iteration of loop, choice function randomly selects letter
            row = [choice(string.ascii_uppercase) for i in range(5)]
#--append new list - row - to board; a.b. so is this a list inside a list?
            board.append(row)
#--send back and store list 'board'
        return board

    def check_valid_word(self, board, word):
        """Check if a word is a valid word in the dictionary and/or the boggle board"""
#--check if word passed in is in words file
        word_exists = word in self.words
#--call find on self object; where is find method? defined at bottom of page
